[CmdletBinding()]
param(
    [string]$BuildId
)

$ErrorActionPreference = "Stop"
$repoRoot = Split-Path -Parent $PSScriptRoot
$outputDir = Join-Path $repoRoot "build\driver-stage-probe"

. (Join-Path $PSScriptRoot "common.ps1")
if (-not $BuildId) {
    $BuildId = Get-V9xBuildId -RepoRoot $repoRoot -Fallback "driver-stage-local"
}
if ($BuildId -notmatch '^[A-Za-z0-9._+-]+$') {
    throw "BuildId may contain only letters, digits, dot, underscore, plus, and hyphen."
}

$watcomRoot = $env:WATCOM
if (-not $watcomRoot -and (Test-Path -LiteralPath "C:\WATCOM")) {
    $watcomRoot = "C:\WATCOM"
}
if (-not $watcomRoot) {
    throw "Open Watcom was not found. Set WATCOM or install it at C:\WATCOM."
}

$compiler = Join-Path $watcomRoot "binnt64\wcc386.exe"
$linker = Join-Path $watcomRoot "binnt64\wlink.exe"
$dumper = Join-Path $watcomRoot "binnt64\wdump.exe"
$kernelLibrary = Join-Path $watcomRoot "lib386\nt\kernel32.lib"
$userLibrary = Join-Path $watcomRoot "lib386\nt\user32.lib"
$requiredInputs = @($compiler, $linker, $dumper, $kernelLibrary, $userLibrary)
$missingInputs = @($requiredInputs | Where-Object {
    -not (Test-Path -LiteralPath $_)
})
if ($missingInputs.Count -ne 0) {
    throw "Required driver-stage build inputs are missing: $($missingInputs -join ', ')"
}

$env:WATCOM = $watcomRoot
$env:Path = "$(Join-Path $watcomRoot 'binnt64');$(Join-Path $watcomRoot 'binnt');$env:Path"
$env:INCLUDE = "$(Join-Path $watcomRoot 'h');$(Join-Path $watcomRoot 'h\nt')"
New-Item -ItemType Directory -Force -Path $outputDir | Out-Null

$source = Join-Path $repoRoot "tools\diag\driver_stage_win32.c"
$object = Join-Path $outputDir "driver_stage_win32.obj"
$executable = Join-Path $outputDir "v9xstage.exe"
$mapFile = Join-Path $outputDir "v9xstage.map"
$linkFile = Join-Path $outputDir "v9xstage.lnk"

& $compiler "-bt=nt" "-zq" "-wx" "-zl" "-s" `
    "-dV9X_BUILD_ID=`"$BuildId`"" "-fo=$object" $source
if ($LASTEXITCODE -ne 0) {
    throw "Open Watcom failed to compile the driver-stage test."
}

Set-Content -LiteralPath $linkFile -Encoding Ascii -Value @(
    "format windows nt",
    "runtime windows=4.0",
    "option quiet",
    "option nodefaultlibs",
    "option start='_V9xDriverStageEntry@0'",
    "option stack=65536",
    "option map='$mapFile'",
    "name '$executable'",
    "file '$object'",
    "library '$kernelLibrary'",
    "library '$userLibrary'"
)
& $linker "@$linkFile"
if ($LASTEXITCODE -ne 0) {
    throw "Open Watcom failed to link the driver-stage test."
}

$bytes = [System.IO.File]::ReadAllBytes($executable)
$newHeaderOffset = if ($bytes.Length -ge 64) {
    [System.BitConverter]::ToInt32($bytes, 0x3c)
} else { -1 }
if ($bytes.Length -lt 64 -or $bytes[0] -ne 0x4d -or
    $bytes[1] -ne 0x5a -or $newHeaderOffset -lt 0 -or
    $newHeaderOffset + 1 -ge $bytes.Length -or
    $bytes[$newHeaderOffset] -ne 0x50 -or
    $bytes[$newHeaderOffset + 1] -ne 0x45) {
    throw "The driver-stage output is not an MZ/PE executable."
}
$imageText = [System.Text.Encoding]::ASCII.GetString($bytes)
foreach ($marker in @($BuildId, "V9X-STAGE begin", "V9X-STAGE PASS")) {
    if (-not $imageText.Contains($marker)) {
        throw "The driver-stage output is missing marker $marker."
    }
}

$dumpText = (@(& $dumper -e $executable 2>&1)) -join "`n"
if ($LASTEXITCODE -ne 0) {
    throw "Open Watcom could not inspect the driver-stage test."
}
$expectedImports = @(
    "CloseHandle", "CreateFileA", "CreateProcessA", "ExitProcess",
    "FlushFileBuffers", "GetCommState", "GetExitCodeProcess", "MessageBoxA",
    "SetCommState", "SetCommTimeouts", "WaitForSingleObject", "WriteFile"
)
foreach ($import in $expectedImports) {
    if ($dumpText -notmatch "(?m)\s$([regex]::Escape($import))\s*$") {
        throw "The driver-stage test is missing import $import."
    }
}
$dllNames = [regex]::Matches($dumpText, "DLL name = <([^>]+)>") |
    ForEach-Object { $_.Groups[1].Value.ToUpperInvariant() } |
    Sort-Object -Unique
$unexpectedDlls = @($dllNames | Where-Object {
    $_ -notin @("KERNEL32.DLL", "USER32.DLL")
})
if ($unexpectedDlls.Count -ne 0 -or
    $dumpText -match "GetCommandLineW|GetModuleFileNameW|__CHK") {
    throw "The driver-stage test contains an incompatible runtime import."
}

Write-Output "Built consolidated driver-stage test: $executable"
Write-Output "Verified runtime-free imports: $($dllNames -join ', ')"
