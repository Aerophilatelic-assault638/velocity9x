...
if ($missingInputs.Count -ne 0) {
    throw "Required trace-dump inputs are missing: $($missingInputs -join ', ')"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Open Watcom failed to compile the trace dump tool."
}
...
$linkLines = @(
    ...
    "option start='_V9xTraceDumpEntry@0'",
    "option stack=65536",
    "option map='$mapFile'",
    "name '$executable'",
    "file '$object'"
...
& $linker "@$linkFile"
if ($LASTEXITCODE -ne 0) {
    throw "Open Watcom failed to link the trace dump tool."
}

$dumpText = (@(& $dumper -e $executable 2>&1)) -join "`n"
if ($LASTEXITCODE -ne 0) {
    throw "Open Watcom could not inspect the trace dump tool."
}
